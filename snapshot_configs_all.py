...
def snapshot_configs():
    ...
    git = photon.git_handler(settings['configs']['local'], remote_url=settings['configs']['remote'])
    git.cleanup

    if not photon.settings.load('queue', settings['configs']['queue']):
        photon.m('could not load snapshot queue from git', more=dict(queue=settings['configs']['queue']), state=True)
    photon.s2m
    ...
    crontab = photon.m('getting crontab', cmdd=dict(cmd='crontab -l'), critical=False)
    if crontab.get('returncode') == 0: write_file(path.join(settings['configs']['target'], 'crontab_-l'), crontab.get('out'))
