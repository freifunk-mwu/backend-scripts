...
if __name__ == '__main__':
    ...
    p = Photon(config='exitping_config.yaml', summary='exitping_summary.yaml', meta='exitping_meta.yaml', verbose=False)
    s = p.settings.get
    ...
    for community in s['batman']:
        ...
        if uping.status['ratio'] <= s['ping']['min_ratio'] or aping.status['ratio'] <= s['ping']['min_ratio']:
            p.m(
                'it seems you are not properly connected!\nremoving batman server flag for %s' %(cif),
